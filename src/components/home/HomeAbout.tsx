...
const DEFAULTS = {
  eyebrow: 'Who We Are',
  headlinePre: 'Leading the next wave of',
  headlineHighlight: 'pharmacy pioneers',
  headlinePost: 'at Nagpur.',
  description:
  ...
  bgVideoUrl: '',
  tags: ['Anti-Ragging', 'Healthcare Drives', 'NotesDrive', 'AURA Symposium', 'Cultural Fests'],
};
...
export const HomeAbout: React.FC<HomeAboutProps> = ({ counts }) => {
    ...
    const fetchContent = async () => {
      try {
        ...
        if (data) {
          ...
          setContent({
            eyebrow: map['about_eyebrow'] || DEFAULTS.eyebrow,
            headlinePre: map['about_headline_pre'] || DEFAULTS.headlinePre,
            headlineHighlight: map['about_headline_highlight'] || DEFAULTS.headlineHighlight,
            headlinePost: map['about_headline_post'] || DEFAULTS.headlinePost,
            description: map['about_description'] || DEFAULTS.description,
            bgVideoUrl: map['about_bg_video_url'] || '',
            tags: (map['about_tags'] || DEFAULTS.tags.join(','))
              .split(',')
              .map((t) => t.trim())
              .filter(Boolean),
          });
        ...
        console.error('HomeAbout settings fetch error:', err);
      }
    };
    fetchContent();
  }, []);
  ...
  return (
    <div className="relative" data-testid="home-about-section">
      {/* Optional admin-controlled background video */}
      {content.bgVideoUrl && (
        <div className="absolute inset-0 overflow-hidden z-0 pointer-events-none">
          ...
          />
          <div className="absolute inset-0 bg-gradient-to-b from-[#050B18]/85 via-[#050B18]/70 to-[#050B18]/90" />
        </div>
      )}
